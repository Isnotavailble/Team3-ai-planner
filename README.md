...
* **Page-Level Workspace Loaders**: Switching session histories or completing onboarding triggers a page-wide shimmering loading state (mimicking KPI cards, financials/budgeting grids, sales charts, and graph canvas) for 1000ms.
...
* **`/` (Root)**: Renders the [`Onboarding.jsx`](file:///c:/Users/AnyaWalker/Desktop/GeminiPlayGround/hackathon/strategic-planner/src/components/Onboarding/Onboarding.jsx) component. Users can input details manually or complete an interactive AI chat onboarding sequence.
...
* **`/workspace/predict`**: Renders the dedicated full-page [`AIReportPage.jsx`](file:///c:/Users/AnyaWalker/Desktop/GeminiPlayGround/hackathon/strategic-planner/src/components/AIReportPage.jsx) simulation suite.
...
1. **CSS Variables & Standard Imports ([`index.css`](file:///c:/Users/AnyaWalker/Desktop/GeminiPlayGround/hackathon/strategic-planner/src/index.css))**: Defines the semantic design token values and imports Tailwind packages using standard `@import` statements to prevent editor warnings. Contains shimmer loader styles.
2. **Tailwind CSS**: Manages responsive structures, flexboxes, spacing, and typography sizes.
3. **Micro-animations & Mesh Gradients**: Built-in CSS animations (`customPulse`, custom scrollbars, bouncing typing indicator) and Framer Motion spring-based configurations for panel transitions.
